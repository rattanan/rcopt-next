...
export function NavigationMenu({ items }: Props) {
  ...
  return <>
    <nav className="desktop-navigation items-center gap-1" aria-label="เมนูหลัก">
    ...
    <button className="icon-button mobile-navigation-trigger" type="button" aria-label={isOpen ? "ปิดเมนู" : "เปิดเมนู"} aria-expanded={isOpen} onClick={() => setIsOpen((open) => !open)}>
      {isOpen ? <X size={21} /> : <Menu size={21} />}
    </button>
    {isOpen && <nav className="mobile-menu mobile-navigation-panel" aria-label="เมนูหลักสำหรับมือถือ">
      {items.map((item) => item.separator ? null : (
