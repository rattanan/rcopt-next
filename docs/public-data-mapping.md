...
|---|---|---|---|
| Doctor | `tbl_profiles.user_id` + `tbl_users.id` | Display: `title`, `firstname`, `lastname`, `licn`; active rule: `profiles.memtype=2 AND users.status=1`. Specialty: `urprf010.crby_tbl_users -> urprf011.id`; workplace: `urprf030.crby_tbl_users -> cmcom010.id` | 1,379 active records. `title`, `licn`, specialty and workplace may be null. Thai stored as UTF-8/utf8mb4. |
...
- Article cover: `images/uploads/<arart010.pimg>`; banner: `images/banner/<cmban010.pimg>`.
...
- The browser never receives `LEGACY_UPLOAD_PATH`; it is retained only for deployment/mount configuration. `LEGACY_ASSET_BASE_URL` is the public origin configuration.
