...
| Medium | Public health discussions can contain personally identifying information. | Public posting now requires consent, CAPTCHA bound to the client IP, and a three-per-hour submission limit; questions are stored disabled pending moderation. Replies require an active doctor session (`memtype=2`). Preserve redaction on all public views. |
